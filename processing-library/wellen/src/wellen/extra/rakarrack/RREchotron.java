...
public class RREchotron implements EffectMono, EffectStereo {
    ...
    public RREchotron() {
        ...
        maxx_size = (int) (RRUtilities.SAMPLE_RATE * 6);   //6 Seconds delay time
    ...
    public void cleanup() {
        RRUtilities.memset(lxn, 0, maxx_size);
        RRUtilities.memset(rxn, 0, maxx_size);
    ...
    public void out(float[] smpsl, float[] smpsr) {
        ...
        for (int i = 0; i < RRUtilities.PERIOD; i++) {
            ...
            intmodl = RRUtilities.lrintf(tmpmodl);
            intmodr = RRUtilities.lrintf(tmpmodr);
    ...
    public void changepar(int npar, int value) {
        ...
        switch (npar) {
            ...
            case PARAM_TEMPO:
                ...
                lfo.Pfreq = RRUtilities.lrintf(subdiv_fmod * tmptempo);
                dlfo.Pfreq = RRUtilities.lrintf(subdiv_dmod * tmptempo);
                lfo.updateparams();
                initparams = true;
                break;
            case PARAM_HIDAMP:
            ...
            case PARAM_LRCROSS:
                ...
                ilrcross = 1.0f - RRUtilities.abs(lrcross);
                break;
            case PARAM_FILTER_PRESETS:
                setfile(value);
                break;
    ...
    private void init_params() {
        ...
        dlyrange = 0.008f * RRUtilities.powf(2.0f, 4.5f * depth);
        ...
        for (int i = 0; i < Plength; i++) {
            tmp_time = RRUtilities.lrintf(fTime[i] * tempo_coeff * RRUtilities.SAMPLE_RATE);
            ...
            if ((tfcnt < ECHOTRON_MAXFILTERS) && (iStages[i] >= 0)) {
                int Freq = (int) (fFreq[i] * RRUtilities.powf(2.0f, depth * 4.5f));
                if (Freq < 20) {
                    Freq = 20;
                }
                if (Freq > hSR) {
    ...
    private void modulate_delay() {
        ...
        float dlfol;
        float dlfor;
        float fperiod = 1.0f / RRUtilities.fPERIOD;

        RRUtilities.SampleStereo lfo_sample = new RRUtilities.SampleStereo();
        RRUtilities.SampleStereo dlfo_sample = new RRUtilities.SampleStereo();
        ...
        if (Pmodfilts) {
            lfmod = RRUtilities.powf(2.0f, (lfol * width + 0.25f + depth) * 4.5f);
            rfmod = RRUtilities.powf(2.0f, (lfor * width + 0.25f + depth) * 4.5f);
            for (int i = 0; i < ECHOTRON_MAXFILTERS; i++) {
        ...
        if (Pmoddly) {
            ...
            ldmod = RRUtilities.lrintf(dlyrange * tempo_coeff * RRUtilities.SAMPLE_RATE * ldmod);
            rdmod = RRUtilities.lrintf(dlyrange * tempo_coeff * RRUtilities.SAMPLE_RATE * rdmod);
    ...
    private int setfile(int value) {
        ...
        for (int count = 1; count < mFilePreset.length; count++) {
            ...
            if ((tTime < -6.0) || (tTime > 6.0f)) {
                ...
                fTime[count] = RRUtilities.fabs(tTime);
            }
            ...
            if ((tiStages < 0) || (tiStages > RRUtilities.MAX_FILTER_STAGES)) {
                error_num = 13;
                break;
            } else {
