...
//        decay_ = fmax(decay, 0.0f);
    ...
    public void SetSnappy(float snappy) {
        snappy_ = DaisySP.fclamp(snappy, 0.f, 1.f);
    }
    ...
    public float Process(boolean trigger) {
        ...
        final float q = 2000.0f * DaisySP.powf(2.f, DaisySP.kOneTwelfth * decay_xt * 84.0f);
        final float noise_envelope_decay = 1.0f - 0.0017f * DaisySP.powf(2.f,
        ...
        DaisySP.fclamp(f_noise, 0.0f, 0.499f);
        ...
        float noise = 2.0f * DaisySP.randf() - 1.0f;
        if (noise < 0.0f) {
            noise = 0.0f;
        }
