...
//        waveform_ = wf < WAVE_LAST ? wf : WAVE_SIN;
    ...
    public float Process() {
        ...
        switch (waveform_) {
            case WAVE_SIN:
                out = DaisySP.sinf(phase_);
                break;
            case WAVE_TRI:
                t = -1.0f + (2.0f * phase_ * TWO_PI_RECIP);
                out = 2.0f * (DaisySP.fabsf(t) - 0.5f);
                break;
            case WAVE_SAW:
            ...
            case WAVE_RAMP:
                ...
                break;
            case WAVE_SQUARE:
                out = phase_ < DaisySP.PI_F ? (1.0f) : -1.0f;
                break;
            case WAVE_POLYBLEP_TRI:
                t = phase_ * TWO_PI_RECIP;
                out = phase_ < DaisySP.PI_F ? 1.0f : -1.0f;
                out += Polyblep(phase_inc_, t);
                out -= Polyblep(phase_inc_, DaisySP.fmodf(t + 0.5f, 1.0f));
                // Leaky Integrator:
                // y[n] = A + x[n] + (1 - A) * y[n-1]
        ...
        phase_ += phase_inc_;
        if (phase_ > DaisySP.TWOPI_F) {
            phase_ -= DaisySP.TWOPI_F;
            eoc_ = true;
        } else {
            eoc_ = false;
        }
        eor_ = (phase_ - phase_inc_ < DaisySP.PI_F && phase_ >= DaisySP.PI_F);
