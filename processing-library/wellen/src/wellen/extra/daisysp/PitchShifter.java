...
public class PitchShifter {
    ...
    public void Init(float sr) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            d_[i].Init();
            phs_[i] = new Phasor();
            phs_[i].Init(sr, 50, i == 0 ? 0 : DaisySP.PI_F);
        }
        shift_up_ = true;
    ...
    public float Process(float in) {
        ...
        gain_[0] = DaisySP.sinf(fade1 * DaisySP.PI_F);
        gain_[1] = DaisySP.sinf(fade2 * DaisySP.PI_F);
    ...
    public float SetTransposition(float transpose) {
        ...
        if (transpose_ != transpose || force_recalc_) {
            ...
            idx = (int) DaisySP.fabsf(transpose);
            ratio = semitone_ratios_[idx % 12];
            ratio *= (int) (DaisySP.fabsf(transpose) / 12) + 1;
            if (transpose > 0.0f) {
                shift_up_ = true;
            } else {
                shift_up_ = false;
    ...
    private void SetSemitones() {
        for (int i = 0; i < 12; i++) {
            semitone_ratios_[i] = DaisySP.powf(2.0f, (float) i / 12);
        }
    }
