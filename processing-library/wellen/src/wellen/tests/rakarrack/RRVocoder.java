...
//        tmpaux = new float[PERIOD];
    ...
    public void out(float[] smpsl, float[] smpsr) {
        final float[] tmpl = new float[PERIOD];
        final float[] tmpr = new float[PERIOD];
        final float[] tsmpsl = new float[PERIOD];
        final float[] tsmpsr = new float[PERIOD];
        final float[] tmpaux = new float[PERIOD];

        float tempgain;
        float maxgain = 0.0f;
        float auxtemp, tmpgain;

        memcpy(tmpaux, auxresampled, auxresampled.length);
        ...
        memcpy(tsmpsl, smpsl, smpsl.length);
        memcpy(tsmpsr, smpsr, smpsr.length);

        memset(tmpl, 0, PERIOD);
        memset(tmpr, 0, PERIOD);
        ...
        memcpy(smpsl, tmpl, PERIOD);
        memcpy(smpsr, tmpr, PERIOD);
//        memcpy(efxoutl, tmpl, PERIOD);
//        memcpy(efxoutr, tmpr, PERIOD);

        vulevel = clamp(rap2dB(maxgain), -48.0f, 15.0f);
    }
