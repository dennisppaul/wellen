...
/**
 * <p>
 * Implementation of The McLeod Pitch Method (MPM). It is described in the article <a href=
 * "http://miracle.otago.ac.nz/tartini/papers/A_Smarter_Way_to_Find_Pitch.pdf" >A Smarter Way to Find Pitch</a>.
 * According to the article:
 * </p>
 * <blockquote> <bufferCount>
 * <p>
 * A fast, accurate and robust method for finding the continuous pitch in monophonic musical sounds. [It uses] a special
 * normalized version of the Squared Difference Function (SDF) coupled with a peak picking algorithm.
 * </p>
 * <p>
 * MPM runs in real time with a standard 44.1 kHz sampling rate. It operates without using low-pass filtering so it can
 * work on sound with high harmonic frequencies such as a violin and it can display pitch changes of one cent reliably.
 * MPM works well without any post processing to correct the pitch.
 * </p>
 * </bufferCount> </blockquote>
 * <p>
 * For the moment this implementation uses the inefficient way of calculating the pitch. It uses <code>O(Ww)</code> with
 * W the window size in samples and w the desired number of ACF coefficients. The implementation can be optimized to
 * <code>O((W+w)log(W+w))</code> by using an <abbr title="Fast Fourier Transform">FFT</abbr> to calculate the <abbr
 * title="Auto-Correlation Function">ACF</abbr>. But I am still afraid of the dark magic of the FFT and clinging to the
 * familiar, friendly, laggard time domain.
 * </p>
 *
...
public final class McLeodPitchMethod implements PitchDetector {
    ...
    /**
     * Overlap defines how much two audio buffers following each other should overlap (in samples). 75% overlap is
     * advised in the MPM article.
     */
    public static final int DEFAULT_OVERLAP = 768;

    /**
     * Defines the relative size the chosen peak (pitch) has. 0.93 means: choose the first peak that is higher than 93%
     * of the highest peak detected. 93% is the default value used in the Tartini user interface.
     */
     ...
     * @param audioSampleRate The sample rate of the audio.
     * @param audioBufferSize The size of one audio buffer 1024 samples is common.
     * @param cutoffMPM       The cutoff (similar to the YIN threshold). In the Tartini paper 0.93 is used.
     */
     ...
     * </p>
     * <p>
     * a,b,c are three samples that follow each other. E.g. a is at 511, b at 512 and c at 513; f(a), f(b) and f(c) are
     * the normalized square difference values for those samples; x is the peak of the parabola and is what we are
     * looking for. Because the samples follow each other
     * <code>b - a = 1</code> the formula for <a
     * href="http://fizyka.umk.pl/nrbook/c10-2.pdf">parabolic interpolation</a> can be simplified a lot.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Implementation based on the GPL'ED code of <a href="http://tartini.net">Tartini</a> This code can be found in the
     * file
     * <code>general/mytransforms.cpp</code>.
     * </p>
     * <p>
     * Finds the highest value between each pair of positive zero crossings. Including the highest value between the
     ...
     * </pre>
     * <p>
     * The array to look for maximum values in. It should contain values between -1 and 1
     *
     * @author Phillip McLeod
     */
