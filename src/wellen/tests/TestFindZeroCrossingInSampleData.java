...
public class TestFindZeroCrossingInSampleData extends PApplet {
    ...
    public static void make_loopable(Sampler mSampler) {
        int[] mAdaptedInOutPoints = find_zero_crossings(mSampler.get_buffer(), mSampler.get_in(), mSampler.get_out());
    ...
    public void draw() {
        ...
        for (int i = mSampler.get_in(); i < mSampler.get_out(); i++) {
            ...
            float y = map(mSampler.get_buffer()[i], -1.0f, 1.0f, 32, height - 32);
            vertex(x, y);
        }
        endShape();
        ...
        fill(0);
        noStroke();
        circle(map(mSampler.get_in(), 0, mSampler.get_buffer().length, 32, width - 32), height / 2.0f, 20);
        circle(map(mSampler.get_out(), 0, mSampler.get_buffer().length, 32, width - 32), height / 2.0f, 20);
    }

    public void keyPressed() {
        switch (key) {
            case '1':
                mSampler.set_in((int) map(mouseX, 0, width, 0, mSampler.get_buffer().length));
                break;
            case '2':
                mSampler.set_out((int) map(mouseX, 0, width, 0, mSampler.get_buffer().length));
                break;
            case '3':
