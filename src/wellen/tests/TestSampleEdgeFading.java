...
public class TestSampleEdgeFading extends PApplet {
    ...
    public void setup() {
        ...
        for (int i = 0; i < mSampler.get_buffer().length; i++) {
            float r = (float) i / mSampler.get_buffer().length;
            r *= TWO_PI;
            r *= 110.0f;
            mSampler.get_buffer()[i] = sin(r) * 0.5f;
        }
        mSampler.enable_loop(true);

        mSampler.set_edge_fading(mSampler.get_buffer().length / 4);

        mFadedData = new float[mSampler.get_buffer().length];
    ...
    public void draw() {
        ...
        stroke(0, 31);
        Wellen.draw_buffer(g, width, height, mSampler.get_buffer(), 10);
        stroke(0, 127);
