 ...
 * TODO(does not sound good … might be something fishy with the random functions) Imitates the sound of dripping water
 */
public class Drip {
    ...
    public void Init(float sample_rate, float dettack) {
        ...
        snd_level_ = 0.0f;
        float tpidsr = 2.0f * DSP.PI_F / sample_rate_;
        ...
        temp = DSP.logf(WUTR_NUM_SOURCES) * WUTR_GAIN / WUTR_NUM_SOURCES;
        gains0_ = gains1_ = gains2_ = temp;
        coeffs01_ = WUTR_RESON * WUTR_RESON;
        coeffs00_ = -WUTR_RESON * 2.0f * DSP.cosf(WUTR_CENTER_FREQ0 * tpidsr);
        coeffs11_ = WUTR_RESON * WUTR_RESON;
        coeffs10_ = -WUTR_RESON * 2.0f * DSP.cosf(WUTR_CENTER_FREQ1 * tpidsr);
        coeffs21_ = WUTR_RESON * WUTR_RESON;
        coeffs20_ = -WUTR_RESON * 2.0f * DSP.cosf(WUTR_CENTER_FREQ2 * tpidsr);
    ...
    public float Process(boolean trig) {
        ...
        if (freq_ != 0.0f && freq_ != res_freq0_) {
            res_freq0_ = freq_;
            coeffs00_ = -WUTR_RESON * 2.0f * DSP.cosf(res_freq0_ * tpidsr);
        }
        ...
        if (freq1_ != 0.0f && freq1_ != res_freq1_) {
            res_freq1_ = freq1_;
            coeffs10_ = -WUTR_RESON * 2.0f * DSP.cosf(res_freq1_ * tpidsr);
        }
        if (freq2_ != 0.0f && freq2_ != res_freq2_) {
            res_freq2_ = freq2_;
            coeffs20_ = -WUTR_RESON * 2.0f * DSP.cosf(res_freq2_ * tpidsr);
        }
        if ((--kloop_) == 0.0f) {
            shake_energy_ = 0.0f;
        }
        ...
        if (gains0_ > 0.001f) {
            ...
            coeffs00_ = -WUTR_RESON * 2.0f * DSP.cosf(center_freqs0_ * tpidsr);
        }
        gains1_ *= WUTR_RESON;
        if (gains1_ > 0.00f) {
            center_freqs1_ *= WUTR_FREQ_SWEEP;
            coeffs10_ = -WUTR_RESON * 2.0f * DSP.cosf(center_freqs1_ * tpidsr);
        }
        gains2_ *= WUTR_RESON;
        if (gains2_ > 0.001f) {
            center_freqs2_ *= WUTR_FREQ_SWEEP;
            coeffs20_ = -WUTR_RESON * 2.0f * DSP.cosf(center_freqs2_ * tpidsr);
        }
    ...
    private long my_random(int max) {
        return ((DSP.rand() % WRAP_RND) % (max + 1));
    }
