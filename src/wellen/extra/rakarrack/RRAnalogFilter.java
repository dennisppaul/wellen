...
public class RRAnalogFilter extends RRFilterI {
    ...
    private boolean oldabovenq;                         //if the last time was above nyquist (used to see if it needs
    // interpolation)
    private final float[] oldc = new float[3];
    private final float[] oldd = new float[3];          // old coefficients(used only if some filter parameters
    // changes very fast,
    private final fstage[] oldx = new fstage[RRUtilities.MAX_FILTER_STAGES + 1];
    private final fstage[] oldy = new fstage[RRUtilities.MAX_FILTER_STAGES + 1];
    ...
    private final fstage[] x = new fstage[RRUtilities.MAX_FILTER_STAGES + 1];
    private final float[] xd = new float[3];
    private final fstage[] y = new fstage[RRUtilities.MAX_FILTER_STAGES + 1];
    ...
    public void setfreq(float frequency) {
        ...
        oldabovenq = abovenq;
        abovenq = frequency > (RRUtilities.SAMPLE_RATE / 2.0f - 500.0);
    ...
    private void computefiltercoefs() {
        ...
        if (freq > (RRUtilities.SAMPLE_RATE / 2.0f - 500.0)) {
            freq = RRUtilities.SAMPLE_RATE * .5f - 500.0f;
            zerocoefs = 1;
        }
        if (freq < 0.1) {
            freq = 0.1f;
    ...
    private void singlefilterout(float[] smp, fstage x, fstage y, float[] c, float[] d) {
        ...
        if (order == 1) {                //First order filter
            for (i = 0; i < smp.length; i++) {
                ...
                y.c1 = y0 + RRUtilities.DENORMAL_GUARD;
                x.c1 = smp[i];
                //output
                smp[i] = y0;
            }
    ...
    private float singlefilterout_s(float smp, fstage x, fstage y, float[] c, float[] d) {
        ...
        if (order == 1) {                //First order filter
            ...
            y.c1 = y0;
            x.c1 = smp + RRUtilities.DENORMAL_GUARD;
            //output
            smp = y0;
