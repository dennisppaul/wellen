...
/**
 * @author Ben Sergentanis brief 808 snare drum model, revisited. date Jan 2021 Ported from
 * pichenettes/eurorack/plaits/dsp/drums/analog_snare_drum.h \n to an independent module. \n Original code written by
...
//        decay_ = fmax(decay, 0.0f);
    ...
    public float Process(boolean trigger) {
        ...
        final float q = 2000.0f * powf(2.f, kOneTwelfth * decay_xt * 84.0f);
        final float noise_envelope_decay = 1.0f - 0.0017f * powf(2.f,
        ...
        for (int i = 0; i < kNumModes; ++i) {
            ...
            shell += gain[i] * (sustain_ ? sin(
            phase_[i] * TWOPI_F) * sustain_gain_value * 0.25f : resonator_[i].Band() + excitation * exciter_leak);
        }
        shell = SoftClip(shell);

        // C56 / R194 / Q48 / C54 / R188 / D54
        float noise = 2.0f * randf() - 1.0f;
        if (noise < 0.0f) {
            noise = 0.0f;
        }
