...
public class FFT extends FourierTransform {
     ...
     * frequencies, but actually represents frequency bands centered on particular frequencies. The center frequency of
     * each band is usually expressed as a fraction of the sampling rate of the time domain signal and is equal to the
     * index of the frequency band divided by the total number of bands. The total number of frequency bands is usually
     * equal to the length of the time domain signal, but access is only provided to frequency bands with indices less
     * than half the length, because they correspond to frequencies below the
     ...
     * Hz, then the spectrum will contain values for frequencies below 22010 Hz, which is the Nyquist frequency (half
     * the sample rate). If you ask for the value of band number 5, this will correspond to a frequency band centered
     * on
     * <code>5/1024 * 44100 = 0.0048828125 * 44100 = 215 Hz</code>. The width of
     * that frequency band is equal to <code>2/1024</code>, expressed as a fraction of the total bandwidth of the
     * spectrum. The total bandwith of the spectrum is equal to the Nyquist frequency, which in this case is 22100, so
     * the bandwidth is equal to about 50 Hz. It is not necessary for you to remember all of these relationships, though
     ...
     * <b>Windowing</b>
     * <p>
     * Windowing is the process of shaping the audio samples before transforming them to the frequency domain. If you
     ...
     * the bandwidth of each average is different. One frequency is an octave above another when it's frequency is twice
     * that of the lower frequency. So, 120 Hz is an octave above 60 Hz, 240 Hz is an octave above 120 Hz, and so on.
     * When octaves are split, they are split based on Hz, so if you split the octave 60-120 Hz in half, you will get
     * 60-90Hz and 90-120Hz. You can see how these bandwidths increase as your octave sizes grow. For instance, the last
     ...
     * will be transformed into a time domain signal and placed in a provided sample buffer. The length of the time
     * domain signal will be <code>timeSize()</code> long. The <code>set</code> and
    ...
    private abstract static class FourierTransform {
        ...
        /**
         * Sets the number of averages used when computing the spectrum and spaces the averages in a linear manner. In
         * other words, each average band will be
        ...
        /**
         * Sets the number of averages used when computing the spectrum based on the minimum bandwidth for an octave and
         * the number of bands per octave. For example, with audio that has a sample rate of 44100 Hz,
         * <code>logAverages(11, 1)</code> will result in 12 averages, each
         * corresponding to an octave, the first spanning 0 to 11 Hz. To ensure that each octave band is a full octave,
         * the number of octaves is computed by dividing the Nyquist frequency by two, and then the result of that by
         * two, and so on. This means that the actual bandwidth of the lowest octave may not be exactly the value
         * specified.
         *
        ...
        /**
         * Sets the window to use on the samples before taking the forward transform. If an invalid window is asked for,
         * an error will be reported and the current window will not be changed.
         *
        ...
        /**
         * Returns the width of each frequency band in the spectrum (in Hz). It should be noted that the bandwidth of
         * the first and last frequency bands is half as large as the value returned by this function.
         *
