...
public class AnalogBassDrum {
    ...
    private float Diode(float x) {
        if (x >= 0.0f) {
            ...
            x *= 2.0f;
            return 0.7f * x / (1.0f + DSP.fabsf(x));
        }
    }
    ...
    public float Process(boolean trigger) {
        ...
        final float q = 1500.0f * DSP.powf(2.f, DSP.kOneTwelfth * decay_ * 80.0f);
        final float tone_f = DSP.fmin(4.0f * f0_ * DSP.powf(2.f, DSP.kOneTwelfth * tone_ * 108.0f), 1.0f);
        ...
        // C40 / R163 / R162 / D83
        pulse_lp_ = DSP.fonepole_return(pulse_lp_, pulse, 1.0f / kPulseFilterTime);
        ...
        fm_pulse_lp_ = DSP.fonepole_return(fm_pulse_lp_, fm_pulse, 1.0f / kPulseFilterTime);
        ...
        if (sustain_) {
            ...
            resonator_out = DSP.sin(DSP.TWOPI_F * phase_) * sustain_gain_;
            lp_out_ = DSP.cos(DSP.TWOPI_F * phase_) * sustain_gain_;
        } else {
        ...
        tone_lp_ = DSP.fonepole_return(tone_lp_, pulse * exciter_leak + resonator_out, tone_f);
