...
public class Sampler implements DSPNodeOutput {
    ...
    public void set_speed(float pSpeed) {
        if (pSpeed > 0) {
            set_frequency(pSpeed * Ton.DEFAULT_SAMPLING_RATE / data().length); /* aka `mStepSize = pSpeed;` */
        } else {
            set_frequency(0);
        }
    }
    ...
    public void set_frequency(float pFrequency) {
        if (mFrequency != pFrequency) {
            ...
            mStepSize = mFrequency * ((float) mData.length / (float) Ton.DEFAULT_SAMPLING_RATE);
        }
    }
